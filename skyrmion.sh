...
# excellent base script write append journalctl command arguments
# test ! $IN_TERMINAL&&{ export IN_TERMINAL=x; urxvt -hold -e "$0" "$@" & exit; } http://www.bashoneliners.com/
# BASH mouse 
about() #== Information about mouse-demo
{
    cat <<EOF
          ...
          Original Author: Chris F.A. Johnson
                   
          This program may be copied under the terms of the
          GNU General Public License, Version 2.
          
          
          Chris F.A. Johnson's excellent bash mouse demo with minor additions...
          
          Edited by: tek Date: 11.08.16 - https://www.gnu.org/licenses/
          
          For those with an interest in working with bash, this demo may be of some use.
          
          oOO^<^OOo regards from the shell... 
...
now()
{
    ...
    eval `date "+DATE=\"%c is the %j day of the year\" YEAR=%Y MONTH=%B DOW=%A DAY=%d TIME=%H:%M:%S HOUR=%H MINUTE=%M SECOND=%S"`
...
help_strings=(
    ...
    "systemd specific critical chain"
